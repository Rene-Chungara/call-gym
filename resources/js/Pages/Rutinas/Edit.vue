...
// Transformar los datos de la rutina para el formulario
const sesionesIniciales = props.rutina.sesiones.map(sesion => ({
    numero_sesion: sesion.numero_sesion,
    descripcion: sesion.descripcion,
    ejercicios: sesion.rutina_sesion_ejercicios.map(ej => ({
        ejercicio_id: ej.ejercicio_id,
        orden: ej.orden,
        series: ej.series,
        repeticiones: ej.repeticiones,
        peso_estimado: ej.peso_estimado,
...
const form = useForm({
    usuario_id: props.rutina.usuario_id,
    nombre: props.rutina.nombre,
    descripcion: props.rutina.descripcion,
    sesiones: sesionesIniciales.length > 0 ? sesionesIniciales : [
        {
            numero_sesion: 1,
            descripcion: "",
            ejercicios: []
...
const submit = () => {
    form.put(route('rutinas.update', props.rutina.id));
};
</script>
            ...
            <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white dark:bg-slate-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        <h1 class="text-2xl font-bold mb-6">Editar Rutina</h1>
                        ...
                        <form @submit.prevent="submit" class="space-y-8">
                            ...
                            <div class="flex gap-3 justify-end pt-6 border-t border-gray-200 dark:border-slate-700">
                                ...
                                <PrimaryButton :disabled="form.processing" class="px-6 py-2">
                                    {{ form.processing ? 'Actualizando...' : 'Actualizar Rutina' }}
                                </PrimaryButton>
                            </div>
                        </form>
                    </div>
